...
│   ├── card_widget.py       # экран карточки (Anki-flip + multiple choice + fill in the blank + matching 6×6)
...
- **Matching (6×6)**: соотнести 6 слов и 6 переводов; упражнение показывается рандомизированно в среднем ~1 раз на 20 упражнений.
...
- **Matching (6×6)** запускается только если движок смог собрать минимум 6 карточек того же типа (слова/конструкции).

### Размер сессии
- Минимальный размер тренировки — **20 карточек** (ограничение действует и в UI, и в `SessionEngine`).
- Если пользователь укажет значение меньше 20 программно, движок всё равно поднимет размер до 20 (если в словаре достаточно карточек).

### Прогресс и matching
- Matching не сдвигает основную очередь карточек, но прогресс по **всем 6 использованным элементам** сохраняется в `progress_*.json`.
- Результат matching считается так:
  - `correct=True`, если все 6 пар собраны без единой ошибки;
  - `correct=False`, если была хотя бы одна неверная попытка.
